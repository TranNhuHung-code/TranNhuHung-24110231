...
import heapq
import random # Cần thiết cho các thuật toán ngẫu nhiên
...
# --- HÀM HEURISTIC 2: SWAP / MISPLACED TILES (Dùng cho Leo Đồi) ---
...
# --- HÀM HỖ TRỢ: SINH TRẠNG THÁI NGẪU NHIÊN HỢP LỆ (Dành cho Restart & Beam) ---
...
# 2. LEO ĐỒI NGẪU NHIÊN (Stochastic Hill Climbing - Theo Ảnh 1)
def stochastic_hill_climbing(start_state, goal_state):
    ...
    while True:
        ...
        # Sinh và lọc tập Better_Neighbors
        for next_state, action in get_neighbors(current_state):
            next_value = -misplaced_tiles(next_state, goal_state)
            if next_value > current_value:
...
# 3. LEO ĐỒI KHỞI ĐỘNG LẠI NGẪU NHIÊN (Random-Restart Hill Climbing - Theo Ảnh 2)
def random_restart_hill_climbing(start_state, goal_state, max_restart=10):
    best_path = []
    
    for i in range(max_restart):
        # Khởi tạo (Restart): Lượt đầu dùng Start gốc, các lượt sau sinh ngẫu nhiên từ Start
        ...
        while True:
            ...
            for next_state, action in get_neighbors(current_state):
                next_value = -misplaced_tiles(next_state, goal_state)
                if next_value > current_value:
                    better_neighbors.append((next_state, action, next_value))
                    
            if not better_neighbors:
                break # Kẹt -> Thoát vòng lặp while, chuyển sang restart tiếp theo
                
            # Chọn trạng thái tốt nhất từ tập Better_Neighbors
            best_next = max(better_neighbors, key=lambda x: x[2])
            current_state = best_next[0]
            path.append((current_state, best_next[1]))
            
        best_path = path # Lưu lại đường đi của lần thử bị kẹt gần nhất
        
    return best_path # Thất bại sau MAX_RESTART lần

# 4. TÌM KIẾM CHÙM CỤC BỘ (Local Beam Search - Theo Ảnh 3)
def local_beam_search(start_state, goal_state, k=3):
    ...
    while True:
        ...
        # SINH TRẠNG THÁI LÂN CẬN của toàn bộ k trạng thái
        ...
        if not neighbor_states_pool:
            return current_states_paths[0][1] # Kẹt hoàn toàn
            
        # LỰA CHỌN CHÙM: Lấy k trạng thái tốt nhất
...
class PuzzleGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("8-Puzzle Solver: Tích hợp đầy đủ Tìm kiếm Cục bộ")
        self.root.geometry("680x680") # Tăng chiều cao để chứa thêm 3 nút
    ...
    def create_widgets(self):
        ...
        # Sắp xếp lại lưới nút bấm cho hợp lý
        buttons_frame = tk.Frame(left_frame)
        buttons_frame.pack(pady=5)
        
        # Hàng 0
        self.btn_ids = tk.Button(buttons_frame, text="Giải bằng IDS", font=('Arial', 10), width=16, command=lambda: self.solve('ids'))
        self.btn_ids.grid(row=0, column=0, pady=3, padx=3)
        self.btn_ucs = tk.Button(buttons_frame, text="Giải bằng UCS", font=('Arial', 10), width=16, command=lambda: self.solve('ucs'))
        self.btn_ucs.grid(row=0, column=1, pady=3, padx=3)
        
        # Hàng 1
        self.btn_greedy = tk.Button(buttons_frame, text="Giải bằng Greedy", font=('Arial', 10), width=16, bg="#f0e68c", command=lambda: self.solve('greedy'))
        self.btn_greedy.grid(row=1, column=0, pady=3, padx=3)
        self.btn_astar = tk.Button(buttons_frame, text="Giải bằng A*", font=('Arial', 10), width=16, bg="#98fb98", command=lambda: self.solve('astar'))
        self.btn_astar.grid(row=1, column=1, pady=3, padx=3)
        
        # Hàng 2: Các nút Leo Đồi
        self.btn_hill = tk.Button(buttons_frame, text="Leo Đồi Đơn Giản", font=('Arial', 10, 'bold'), width=16, bg="#ffb6c1", command=lambda: self.solve('hill'))
        self.btn_hill.grid(row=2, column=0, pady=3, padx=3)
        self.btn_stochastic = tk.Button(buttons_frame, text="Stochastic Hill", font=('Arial', 10, 'bold'), width=16, bg="#ffb6c1", command=lambda: self.solve('stochastic'))
        self.btn_stochastic.grid(row=2, column=1, pady=3, padx=3)
        
        # Hàng 3: Random Restart và Beam
        self.btn_restart = tk.Button(buttons_frame, text="Random-Restart", font=('Arial', 10, 'bold'), width=16, bg="#ffb6c1", command=lambda: self.solve('restart'))
        self.btn_restart.grid(row=3, column=0, pady=3, padx=3)
        self.btn_beam = tk.Button(buttons_frame, text="Local Beam Search", font=('Arial', 10, 'bold'), width=16, bg="#dda0dd", command=lambda: self.solve('beam'))
        self.btn_beam.grid(row=3, column=1, pady=3, padx=3)
